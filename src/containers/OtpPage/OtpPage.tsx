...
const OtpPage: FC<OtpPageProps> = ({ className = "" }) => {
	...
	return (
		<div className={classes.Form}>
			...
			<OtpIcon />
			<h2 className={classes.OtpHeading}>OTP Verification</h2>
			<h3 className={classes.OtpDesc}>Enter the verification code we just sent on your email address.</h3>
			<OtpInput
								value={otp}
							inputStyle={{
	 								height: "3.5rem",
								...
								containerStyle={{ direction: "ltr" }}
	 						/>
							<button className={classes.OtpBtn} onClick={submitHandler}>Verify</button>
							<p className={classes.OtpCode} >Didn't received code?</p>
							<span className={classes.OtbLink}>Resend</span>
			</form>
		</div>
	)
};
